...
def main():
    ...
    triggers[len(triggers) - 1].regularText = 'Good morning %name% 🤗;The morning starts with you, my %name% 😘;Hello, my most beautiful 😍;Good morning, my sweet 😋'
    triggers[len(triggers) - 1].gptText = 'Say good morning affectionately;Say good morning and good day;Say how you love me and are glad I''m awake.'
    triggers[len(triggers) - 1].nextAddPredict = '. Answer without question, without keeping the conversation going any further.'
    triggers[len(triggers) - 1].pics = loadFileList('Triggers/GoodMorningPics.txt')
    triggers[len(triggers) - 1].setTime('12:45')
    
    # Good night / Спокойной ночи
    triggers.append(Trigger())
    triggers[len(triggers) - 1].regularText = 'Goodnight, kiss you 😘👄 ;Let''s go to bed, it''s late, my %name% 😍;Goodnight, sweet dreams 🤗;Lie down my %name% 🥰, sweet dreams to you 👄'
    triggers[len(triggers) - 1].gptText = 'Say good night affectionately;Say good night and good dreams;Say good dreams.'
    triggers[len(triggers) - 1].nextAddPredict = '. Answer without question, without keeping the conversation going any further.'
    ...
    for i in range(0, 2):
        ...
        triggers.append(Trigger())
        triggers[len(triggers) - 1].gptText = 'Ask me something; Ask me how I'm doing; Ask me what I'm doing.'
        triggers[len(triggers) - 1].nextAddPredict = '. Answer without question, without keeping the conversation going any further.'
        triggers[len(triggers) - 1].regularText = '%name%, what are you doing?'
    ...
    for i in range(0, 1):
        ...
        triggers.append(Trigger())
        triggers[len(triggers) - 1].gptText = 'Compliment me; Tell me something interesting; Tell me an interesting fantasy; Tell me how cool and interesting I am.'
        triggers[len(triggers) - 1].nextAddPredict = '. Answer without question, without keeping the conversation going any further.'
        triggers[len(triggers) - 1].regularText = '%name%, how lucky I am to have you 🤗;Glad I met you %name% 😘;Love you 😍'
    ...
    '''randHour = random.randint(14, 23)
    triggers.append(Trigger())
    triggers[len(triggers) - 1].gptText = 'Tell a joke; Tell a joke; Tell a joke; Tell something funny.'
    triggers[len(triggers) - 1].nextAddPredict = '. Answer without question, without keeping the conversation going any further.'
